...
/** We globally define an array of mutex variables
 *CB
*/
pthread_mutex_t** gridLockMutex;
 ...
 */
void movetotarget(Robot &r, int tx, int ty)///////////////////////////////////////////////////////////////////////////////////////////// ROBOTS
{
	//calc vert
	...
	while (vertshift != 0)
	{
		...
		if (vertshift>0)
		{
			// Lock spot to move to
			pthread_mutex_lock(&gridLockMutex[r.x][(r.y - 1)]);

			// Move to locked spot
			r.y--;

			// Unlock Previous spot
			pthread_mutex_unlock(&gridLockMutex[r.x][(r.y + 1)]);
	...
	while (horzshift != 0)
	{
		...
		if (horzshift>0)
		{
			...
			// Lock spot to move to 
			pthread_mutex_lock(&gridLockMutex[(r.x + 1)][r.y]);

			// Move to locked spot
			r.x++;

			// Unlock previous spot
			pthread_mutex_unlock(&gridLockMutex[(r.x - 1)][r.y]);
 ...
 */
void pushboxvert(Robot &r, int vertshift)////////////////////////////////////////////////////////////////////////////////////////////////// BOX VERT
{
	//pushbox
 ...
 */
void pushbothorz(Robot &r, int horizshift)//////////////////////////////////////////////////////////////////////////////////////////////////// BOX HORIZ
{
	//pushbox
...
void movebot(Door d, Box &b, Robot &r)
{
	...
	// Lock Robots starting position
	pthread_mutex_lock(&gridLockMutex[r.x][r.y]);
	...
	// Unlock Robots final Position to let other robots move into that spot
	pthread_mutex_unlock(&gridLockMutex[r.x][r.y]);
...
void initializeApplication(void)
{
	...
	// Allocate 2D array of mutex's then initialize them ///////////////////////////////////////////////////////////////////////////////////////////////////////////
	gridLockMutex = (pthread_mutex_t**) malloc(numRows * sizeof(pthread_mutex_t*));

	for(int i = 0; i < numRows; i++){
		gridLockMutex[i] = (pthread_mutex_t*) malloc(numCols * sizeof(pthread_mutex_t));
	}

	// Initialize 2D Mutex array
	for (int i = 0; i < numRows; i++)// Rows
	{
		for (int j = 0; j < numCols; j++)// Columns
		{
			pthread_mutex_init(&gridLockMutex[j][i], NULL);
		}
